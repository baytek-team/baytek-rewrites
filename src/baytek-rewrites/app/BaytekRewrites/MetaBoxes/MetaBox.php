...
namespace BaytekRewrites\MetaBoxes;
	...
	public function saveMeta( $post_id, $post ) {
    	...
    	if ( check_admin_referer( $this->id , $this->id . '_nonce' ) ) {
    		...
    		foreach ( $this->metaKeys as $metaKey ) {
    			...
    			//If data isn't set, metavalue is false
    			if (!isset($_POST[$metaKey])) {
    				$metaValue = false;
    			}
    			//Handle array
				else if (is_array($_POST[$metaKey])) {
					$metaValue = $_POST[$metaKey];
				}
				//Handle strings, etc.
				else {
			...
			//Save any additional info
			do_action('saving_meta_'.$this->id, $post_id);
    	}
	}
